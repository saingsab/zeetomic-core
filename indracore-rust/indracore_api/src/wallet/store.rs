...
impl Extrinsics {
    ...
    pub fn get(&self, block_number: u32, index: u32) -> Option<&Extrinsic> {
        self.xts
            .iter()
            .find(|xt| xt.block_number == block_number && xt.index == index)
    }
}
...
impl FileStore {
    pub fn get(addr: &str) -> Self {
        ...
        let db = FileDatabase::<Extrinsics, Bincode>::load_from_path_or(file, backend)
    ...
    pub fn update(&self, scanned_at: u32) {
        self.load();
        self.0
            .write(|backend| backend.scanned_at = scanned_at)
